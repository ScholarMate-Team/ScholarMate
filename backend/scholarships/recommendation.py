...
def recommend_final_scholarships_by_gpt(filtered_scholarships_queryset: QuerySet, user_profile: UserScholarship) -> List[Dict]:
    ...
    user_info_dict = user_profile.to_dict() # to_dict() 메서드가 UserScholarship에 있다고 가정
    ...
    **[매우 중요한 규칙]**
    ...
    2.  **구체적인 이유 제시:** 'reason'에는 왜 이 장학금이 사용자에게 적합한지, 어떤 조건(지역, 성적, 소득, 전공, 학년, 특정 자격 등)이 어떻게 부합하는지를 **2~3문장 이상 자연스러운 문단 형태로** 작성하세요.
...
def recommend(user_id: int) -> List[Dict]:
    ...
    try:
        ...
        print(f"오류: UserScholarship 클래스가 정의되지 않았습니다. 실제 모델을 확인하세요.")
        return []

    try:
        scholarships = Scholarship.objects.all()
    except NameError:
        print(f"오류: Scholarship 클래스가 정의되지 않았습니다. 실제 모델을 확인하세요.")
        return []
